...
if (Config.WORKTYPE == 'private') {
    ...
    MyPnky.addCommand({pattern: 'status', fromMe: true, desc: 'download whatsapp status (only video for image take screenshot vro)'}, (async (message, match) => {    

        if (message.reply_message === false) return await message.client.sendMessage(message.jid,'```reply to a status video or photo```', MessageType.text);
        var downloading = await message.client.sendMessage(message.jid,'*Bro.. 💬 I am taking your status 🐼*',MessageType.text);
        var location = await message.client.downloadAndSaveMediaMessage({
            key: {
                remoteJid: message.reply_message.jid,
                id: message.reply_message.id
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                ...
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/WhatsAsenaDuplicated/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: 'Made by Garfield', quoted: message.data })
   ...
   MyPnky.addCommand({pattern: 'mp4$', desc: Lang.ANİM_STİCK, fromMe: false}, (async (message, match) => {
    ...
    var usge = ''
     if (Config.LANG == 'EN') {
        ...
        usge = '.doc Garfield (replace Garfield with desired name)'
    }
    if (Config.LANG == 'ML') {
