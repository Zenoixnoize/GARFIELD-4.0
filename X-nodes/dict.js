...
MyPnky.addCommand({pattern: 'lngcode', fromMe: false, dontAddCommandList: true}, (async (message, match) => {    
    ...
    await message.sendMessage('⚙️ *There are English 🏴󠁧󠁢󠁥󠁮󠁧󠁿 and other languages* \n ⚙️ *But there is no Sinhala language 🇱🇰*  \n\n ```NOIZE PROJECTS``` ');

}));



/*MyPnky.addCommand({pattern: 'dict ?(.*)', fromMe: false,  dontAddCommandList: true }, async (message, match) => {
...
/*MyPnky.addCommand({ pattern: 'dict ?(.*)', fromMe: false, desc: "Use it as a dictionary.\nEg: .dict en_US;lead\n For supporting languages send *.lngcode*" }, async (message, match) => {
