...
const FilterDb = require('./A14/filters');
const Config = require('../config')
const Garfield = require('../Garfield')
const { AfnanPLK, addplk } = require("./A14/6TEA");
const afnp = Garfield.PLKAFN !== false ? Garfield.PLKAFN.split(',') : [];
...
MyPnk.addCommand({pattern: 'xjjddjs ?(.*)', fromMe: true, desc: Lang.FILTER_DESC, dontAddCommandList: true}, (async (message, match) => {
    match = match[1].match(/[\'\"\“](.*?)[\'\"\“]/gsm);
...
const array = ['Garfield','asena','ayin','back','Back','Bot','fuck','Fuck','Hehe','Hello','Hlo','Kill','kill','kiss','line','love','mwolu','Mwolu','single','tha','thund','z','Z','bie','Bie']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
