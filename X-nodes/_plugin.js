...
MyPnky.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        if (plugin_name.length >= 1) {
            plugin_name = "grfld_" + plugin_name[1];
        } else {
            plugin_name = "grfld_" + Math.random().toString(36).substring(8);
        }
        ...
        try {
            ...
            fs.unlinkSync('/root/WhatsAsenaDuplicated/X-nodes/' + plugin_name + '.js')
        ...
        if (response.body.includes('/A14/notes') || response.body.includes('/A14/lydia') || response.body.includes('/A14/plugin') || response.body.includes('/A14/greetings') || response.body.includes('/A14/filters')) DEG.level = DEG.level + 33
        ...
        if (find.includes(plugin_name)) {
            ...
            else if (!match[1].includes('phaticusthiccy') || (!match[1].includes('zenoixnoize') && DEG.level < 100)) {
...
MyPnky.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, dontAddCommandList: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
    ...
    if (!match[1].startsWith('grfld_')) match[1] = 'grfld_' + match[1];
    try {
